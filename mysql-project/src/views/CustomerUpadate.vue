...
<template>
  <div class="contaniner">
    <div class="row">
      ...
      <div class="row">
        <label>이름</label>
        <input type="tel" v-model="info.name">
      </div>
      <div class="row">
        <label>이메일</label>
      ...
      <div>
        <button class="btn btn-info" v-on:click="addCustomer" >
          수정
        </button>
      </div>
    </div>
  ...
  data(){
    return {
      ...
      async updateCustomer() {
        ...
        //수정
        let result = await axios.put(`/api/customers/${this.info.id}`, updateData).catch(err => console.log(err));
        let sqlRes = result.data;
        let affectRows = sqlRes.affectRows;
        if (affectRows > 0){
          alert('정상적으로 수정되었습니다.')
        } else {
